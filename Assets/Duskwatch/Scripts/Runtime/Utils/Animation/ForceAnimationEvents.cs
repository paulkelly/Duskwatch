...
/// </summary>
public class ForceAnimationEvents : StateMachineBehaviour
{
    ...
    public class StateEvent
    {
        ...
        [Tooltip("Should the event be fired? Useful for disabling events for debugging.")] [SerializeField]
        protected bool m_enabled = true;

        [Tooltip("At what point in an StateUpdate should the event fire, only used for OnUpdateEvents.")] [SerializeField] [Range(0f, 1f)]
        protected float m_normalizedTime = 0.5f;

        [Tooltip("Should the event be called regardless if the state is being exited? only used for OnUpdateEvents")] [SerializeField]
        protected bool m_ForceCallOnExit = true;

        [Tooltip("Should the event be called in each loop or just on the first iteration?")] [SerializeField]
        protected bool m_repeatOnLoop = true;

        [Tooltip("Which function to call when sending the event")] [SerializeField]
        protected string m_FunctionName = string.Empty;

        [Tooltip("the type of Parameter to send")] [SerializeField]
        protected ArgumentType m_parameterType = ArgumentType.stringType;
        ...
        {
            get { return m_repeatOnLoop; }
        }

        public bool ForceCallOnExit
        {
            get { return m_ForceCallOnExit; }
        }
        ...
        public ArgumentType ParameterType
        {
        ...
        {
            get { return m_objectParameter; }
            set { m_objectParameter = value; }
        }
    ...
    [SerializeField] private AnimationClip _copyFrom;

#if UNITY_EDITOR
    [Button("PopulateEventsFromClip")] [SerializeField]
    private bool editorFoldout;
#endif

    // Editor Button functions
#if UNITY_EDITOR
    public void PopulateEventsFromClip()
    {
        if (_copyFrom != null)
        {
            OnUpdateEvents = new StateEvent[_copyFrom.events.Length];
            for (int i = 0; i < _copyFrom.events.Length; i++)
            {
                OnUpdateEvents[i] = CreateStateEventFromAnimationEvent(_copyFrom.events[i]);
            }
        }
    }

    private StateEvent CreateStateEventFromAnimationEvent(AnimationEvent animEvent)
    {
        StateEvent.ArgumentType type = StateEvent.ArgumentType.none;
        if (string.IsNullOrEmpty(animEvent.stringParameter))
        {
            type = StateEvent.ArgumentType.stringType;
        }
        else if (animEvent.objectReferenceParameter != null)
        {
            type = StateEvent.ArgumentType.UnityObjectType;
        }
        else if (animEvent.intParameter != 0)
        {
            type = StateEvent.ArgumentType.intType;
        }
        else if (Mathf.Abs(animEvent.floatParameter) > 0.1f)
        {
            type = StateEvent.ArgumentType.floatType;
        }

        return new StateEvent()
        {
            NormalizedTime = animEvent.time / _copyFrom.length,
            ...
            ObjectParameter = animEvent.objectReferenceParameter
        };
    }
#endif
}